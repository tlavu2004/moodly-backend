...
- [~] Use two local Auth0 demo users to verify that neither local MongoDB-backed endpoints nor local Elasticsearch search reveals the other user's data, and neither user can perform Cloudinary asset-management operations for the other's avatar. The Bruno run verified that user B cannot read user A's MongoDB/Search data or confirm user A's pending avatar; complete the reciprocal user-A-versus-user-B, signature-reuse, and deletion attempts before marking this complete.
...
- [~] **Bootstrap and MongoDB isolation.** User A bootstrap, idempotency, habits, entries, statistics, and the user-B-cannot-read-user-A check passed in Bruno. Still create distinct user-B habit/entry data, verify user A cannot read it, and inspect that the two local `users` documents contain distinct `auth0Subject` values only (no credentials, sessions, refresh tokens, or password hashes).
- [~] **CDC/search isolation.** User-A search and the user-B-cannot-find-user-A check passed, as did a full reindex and real dead-letter replay. Still create unique user-B searchable data, repeat the isolation assertion in the opposite direction, inspect indexed `userId` values, and confirm no request can override the caller's user ID.
- [~] **Negative token matrix.** Missing and malformed/invalid token requests returned the standard `401 UNAUTHORIZED` envelope. Still exercise expired token, invalid signature, wrong issuer, and wrong audience using dedicated test credentials or an approved Auth0 test mechanism; confirm `403 FORBIDDEN` occurs only after a valid authenticated identity fails an explicit authorization rule.
...
- [~] **Avatar validation.** Unsupported MIME type and declared size above 5 MiB were rejected before signature issuance. Still test fabricated public ID, altered version, expired pending upload, asset outside the owner namespace, disallowed Cloudinary format, and Cloudinary-confirmed size above 5 MiB, verifying none replaces profile metadata.
...
- [~] **Cross-user asset ownership.** User B could not confirm user A's pending public ID. Still attempt user-A/user-B data isolation in the reciprocal direction, user-A signature reuse by user B, and any avatar overwrite/deletion attempt; user A's metadata and remote asset must remain unchanged.
