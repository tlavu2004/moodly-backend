...
- [ ] Elasticsearch outage and recovery (manual Docker verification): do not automate by pausing the Testcontainers Elasticsearch container on Docker Desktop—the HTTP transport can hang instead of failing deterministically. Stop `moodly-elasticsearch-local` after MongoDB accepts a write; assert the core write succeeds, retry attempts are bounded, a dead letter is stored, and the CDC health details show an increased `failedEventCount` plus the failure metadata. Restore Elasticsearch, replay the dead letter, and await the recovered document; confirm the dead letter is removed and `failedEventCount` decreases. Mark this item `[x]` only after this complete local scenario is recorded as passing.
