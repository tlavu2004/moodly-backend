...
- [x] Extend the `.http` file with an Auth0 access-token acquisition note and authenticated CRUD, search, cross-user isolation, invalid/expired-token, and avatar signed-upload scenarios. Exercise wrong audience with a deliberately minted Auth0 test token during the manual run; cover wrong issuer deterministically in the automated JWT validator suite unless a second tenant is available.
...
- [x] Run the full local stack with `make local-up` and `make local-run`; verify authenticated CRUD, CDC indexing, search, avatar upload, avatar replacement/deletion, and error handling before beginning any frontend work. Verified locally through the Bruno guide: reindex and dead-letter replay, two-way isolation, hosted negative-token checks (except wrong issuer, covered deterministically by the automated validator), avatar validation, and cleanup-retry coverage are complete.
...
- [x] **Negative token matrix.** Missing and malformed requests returned the standard `401 UNAUTHORIZED` envelope. Hosted expired, invalid-signature, and wrong-audience checks returned `401` on 2026-09-12. `SecurityConfigurationTest` rejects wrong issuer, wrong audience, and expired claims; `ApiIntegrationTest` verifies the standard `401` envelope for a structured bearer JWT rejected by the decoder. A second Auth0 tenant is not required for this local checkpoint.
...
- [x] **Avatar validation.** Unsupported MIME type and declared size above 5 MiB were rejected before signature issuance. Verified 2026-09-12: fabricated public ID and altered version both returned `400`, while `GET /me/avatar` retained the prior confirmed metadata. `AvatarServiceTest` covers the remaining expired-pending, namespace, confirmed-format, and confirmed-size rejection paths without profile mutation.
- [x] **Replacement and cleanup.** A second user-A avatar replaced the first and deleted the previous Cloudinary asset; an abandoned upload was cleaned after expiry. `PendingAvatarUploadCleanupTest` simulates remote deletion failure, verifies that the pending record remains with an incremented cleanup attempt count, and separately verifies successful cleanup removes both remote asset and pending record.
- [x] **Cross-user asset ownership.** Verified 2026-09-12: with a fresh unconfirmed user-A upload, User B confirmation returned `400` at the namespace ownership check and `GET /me/avatar` for user A remained unchanged. The signature endpoint has no caller-supplied public-ID input and no avatar overwrite/delete API exists; the short-lived Cloudinary signature is a fixed-public-ID bearer capability rather than an Auth0 authorization primitive.
